...
const { createBriefFromTemplate, updateBriefFromTemplate, getAllTemplates, getTemplateById, getAllTemplatesPreview } = require('../services/template.services');
...
router.put('/from-template', authenticateAdmin, async (req, res) => {
  try {
    ...
    if (!template.id || typeof template.id !== 'string') {
      return res.status(400).json({ error: 'Brief ID is required for update' });
    }
    ...
    const brief = await updateBriefFromTemplate(template.id, template, title);
